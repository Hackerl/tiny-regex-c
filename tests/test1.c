...
int main() {
    ...
    for (i = 0; i < ntests; ++i) {
        ...
        if (should_fail) {
            if (m != (-1)) {
                printf("\n");
                re_print(re_compile(pattern, re_compiled, ccl_buf));
                fprintf(stderr, "[%lu/%lu]: pattern '%s' matched '%s' unexpectedly, matched %i chars. \n", (i + 1),
                        ntests, pattern, text, length);
                nfailed += 1;
            }
        } else {
            if (m == (-1)) {
                ...
                fprintf(stderr, "[%lu/%lu]: pattern '%s' matched '%i' chars of '%s'; expected '%i'. \n", (i + 1),
                        ntests, pattern, length, text, correctlen);
                nfailed += 1;
            }
        }
    }
    ...
    return (int) nfailed; /* 0 if all tests passed */
}
